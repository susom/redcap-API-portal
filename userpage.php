...
if (!informedConsented($record, REDCAP_API_TOKEN, REDCAP_API_URL)) {

   echo "</br>Surveys are not available until you have completed the consent form.";
   $link = getSurveyLink($record, 'informed_consents', EVENT_1, REDCAP_API_TOKEN, REDCAP_API_URL);
   //logIt( "This is the LINK".$link, "DEBUG");
   echo "  <a href=$link class=\"list-group-item \">Informed consent</a>";
    
  // echo "</br><a href=$link > Informed consent</a>";
} else {

   $instrument_list = array_keys($learn_forms);
   $result = getAllCompletionStatus($record,$instrument_list,EVENT_1, REDCAP_API_TOKEN, REDCAP_API_URL );
...
//    $emails = retrieveEmailsSent($record, REDCAP_API_TOKEN, REDCAP_PORTAL_EMAIL_API);
//    logIt("EMAILS: " . print_r($emails, true));
   ...
   foreach ($learn_forms as $form=>$form_data) {
	...
	if (array_key_exists('retake', $form_data)) {
			$retakeHash = getRetakeHash($record, $form, EVENT_1, REDCAP_API_TOKEN, REDCAP_API_URL);
			$link = getSurveyLink($record, $form, EVENT_1, REDCAP_API_TOKEN, REDCAP_API_URL);
...
//		 echo "  <li class=\"list-group-item disabled \">$label</li>";
        ...
        // echo "</br>".$label;
		 ...
		 if ($form == 'pregnancy_form') {
		    $pregnant = getFieldStatus($record, 'pregnancy',EVENT_1,  REDCAP_API_TOKEN, REDCAP_API_URL);
		 ...
		 if ($form == 'family_members_your_generation_siblings') {
			$siblings = getFieldStatus($record, 'siblings_exist', EVENT_1,  REDCAP_API_TOKEN, REDCAP_API_URL);
		 ...
		 if ($form == 'family_members_other_affected_relatives') {
			$relatives = getFieldStatus($record, 'have_family_info', EVENT_1,  REDCAP_API_TOKEN, REDCAP_API_URL);
...
<div class='container'>
   <div class="row">
      <div class="jumbotron">
        ...
        <p>Please check back in approximately six months to update
your records about any new developments in your medical condition that
may have occurred.</p>  
...
//	alert("starting redirect from onclick");
	
//   var redirectHash1 = $('span', caller.attr('redirect'));
   var redirectHash = caller.attr('redirect');
...
//	$("#resumeSurveyForm").attr("action", link).submit();​    //why doesn't this work?
